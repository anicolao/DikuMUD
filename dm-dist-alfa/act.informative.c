...
void do_score(struct char_data *ch, char *argument, int cmd)
{
	...
	sprintf(buf, 
		"You have %d(%d) hit, %d(%d) flux and %d(%d) movement points.\n\r",
...
void do_levels(struct char_data *ch, char *argument, int cmd)
{
	...
	if (*arg) {
		if (!strcasecmp(arg,"scientist")) class = 1;
		else if (!strcasecmp(arg,"noble")) class =  2;
		else if (!strcasecmp(arg,"assassin")) class =  3;
		else if (!strcasecmp(arg,"warrior")) class = 4;
	}
 ...
 * We match quests to zones by checking if the giver mob's vnum matches the
 * zone's primary vnum prefix (e.g., zone with rooms 3001-3055 matches mobs
 * 3000-3099, as both are in the "30xx" range).
 * 
 * Quests are attributed to the zone where they can be FOUND (where the quest
 * giver is located), not where they need to be completed. This means a quest
 * with giver in Lesser Helium counts as a Lesser Helium quest, regardless of
 * where the quest objectives are located.
 */
                               ...
                               int *total, int *completed)
{
	...
	*completed = 0;
	...
	for (q = 0; q < top_of_quest_table; q++) {
		/* Check if the quest giver vnum matches this zone's prefix.
		 * A quest is attributed to the zone where it can be found (where the
		 * giver is), not based on the quest number or completion location.
		 */
...
void do_zone(struct char_data *ch, char *argument, int cmd)
{
	...
	if (!*arg) {
		...
		if (zone_table[zone_num].top <= 3) {
			send_to_char("You are in a system zone.\n\r", ch);
			return;
		}
		if (!strcasecmp(zone_table[zone_num].name, "zone_1200")) {
			send_to_char("You are in zone_1200.\n\r", ch);
			return;
		}
		
		/* Calculate zone range */
		zone_start = (zone_num == 0) ? 0 : (zone_table[zone_num-1].top + 1);
		zone_end = zone_table[zone_num].top;
		
		/* Count rooms in this zone */
		num_rooms = 0;
		for (r = 0; r < top_of_world; r++) {
			if (world[r].zone == zone_num) {
				num_rooms++;
			}
		}
		...
		for (mob = character_list; mob; mob = mob->next) {
			if (IS_NPC(mob) && mob->in_room != NOWHERE) {
				if (world[mob->in_room].zone == zone_num) {
					num_mobs++;
				}
			}
		}
		
		strcpy(buf, "Zone Information:\n\r");
		strcat(buf, "=================\n\r\n\r");
		
		snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
			"Name: %s\n\r", zone_table[zone_num].name);
		snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf),
			"Rooms: %d  Active Mobs: %d\n\r", num_rooms, num_mobs);
		
		/* Add level range info for known zones */
		if (!strcasecmp(zone_table[zone_num].name, "Lesser Helium")) {
			strcat(buf, "Level Range: 1-10 (Starting zone)\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Lesser Helium Sewers")) {
			strcat(buf, "Level Range: 1-4\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Dead Sea Wilderness")) {
			strcat(buf, "Level Range: 5-15\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Greater Helium")) {
			strcat(buf, "Level Range: 8-16\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Zodanga - Enemy City")) {
			strcat(buf, "Level Range: 10-18\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Zodanga Wilderness")) {
			strcat(buf, "Level Range: 14-20\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Gathol - Allied City")) {
			strcat(buf, "Level Range: 12-20\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Ptarth - Allied City")) {
			strcat(buf, "Level Range: 15-20\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Kaol - Allied City")) {
			strcat(buf, "Level Range: 16-20\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Thark Territory")) {
			strcat(buf, "Level Range: 3-6\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Atmosphere Factory - Main Levels")) {
			strcat(buf, "Level Range: 4-8\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Atmosphere Factory - Lower Levels")) {
			strcat(buf, "Level Range: 8-12\n\r");
		} else if (!strcasecmp(zone_table[zone_num].name, "Southern Approach to Atmosphere Factory")) {
			strcat(buf, "Level Range: 3-6\n\r");
		}
